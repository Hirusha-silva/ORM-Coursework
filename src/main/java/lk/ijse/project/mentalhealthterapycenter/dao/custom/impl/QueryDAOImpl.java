...
public List<ViewSessionDTO> getAllAppointments() {
    ...
    try {
        ...
        for (Object row : resultList) {
            ...
            String doctorID = (String) result[4];
            String patientName = (String) result[5];
            String paymentID = result[6] != null ? result[6].toString() : null;
            Double paymentAmount = result[7] != null ? ((Number) result[7]).doubleValue() : null;
            String paymentMethod = (String) result[8];
            String appointmentStatus = (String) result[9];
            String programConcat = (String) result[10];
            ...
            // Add split program IDs (from GROUP_CONCAT) to the session's program list
            if (programConcat != null && !programConcat.isEmpty()) {
                String[] programIDs = programConcat.split(",");
                sessionDTO.getPrograms().addAll(Arrays.asList(programIDs));
            }
        }
